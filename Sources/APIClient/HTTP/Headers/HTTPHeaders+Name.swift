public extension HTTPHeaders {
    ...
    struct Name: Codable, Hashable, Equatable, CustomStringConvertible {
        /// See `Hashable`
        public func hash(into hasher: inout Hasher) {
            lowercased.hash(into: &hasher)
        }
    ...
    func contains(name: Name) -> Bool {
        contains(name: name.lowercased)
    }

    /// Returns the first header value with the supplied name.
    /// - Parameter name: The header field name whose values are to be retrieved.
    func first(name: Name) -> String? {
        first(name: name.lowercased)
    }

    subscript(canonicalForm name: Name) -> [Substring] {
